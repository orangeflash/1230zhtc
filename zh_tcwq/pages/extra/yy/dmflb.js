...
Page({
  ...
  yyxminput(e) {
    this.setData({
      [`storelist[${e.currentTarget.dataset.index}].receivable`]: e.detail.value
    })
    // console.log(e)
  },
  tjxm(e) {
    if (this.data.storelist[e.currentTarget.dataset.index].receivable <= 0) return wx.showToast({
      title: '请输入应收金额',
      icon: 'none',
      duration: 2000
    })
    var a = this, oid = this.data.storelist[e.currentTarget.dataset.index].id, receivable = this.data.storelist[e.currentTarget.dataset.index].receivable;
    console.log(oid)
    wx.showModal({
      title: "提示",
      content: "是否修改应收金额？",
      ...
      success: function (s) {
        ...
        s.confirm && (wx.showLoading({
          ...
          'url': 'entry/wxapp/SaveDmOrder',
          data: { id: oid, receivable, },
          success: function (res) {
            console.log(res.data)
            if (res.data == '1') {
              wx.showToast({
  ...
  reLoad: function () {
    ...
    app.util.request({
      ...
      'cachetime': '0',
      data: { is_tg: qgstate, storeId: store_id, page: page, pagesize: 10 },
      success: function (res) {
        // console.log('分页返回的列表数据', res.data)
        for (let i = 0; i < res.data.length; i++) {
          res.data[i].qkje = res.data[i].receivable-res.data[i].money
        }
